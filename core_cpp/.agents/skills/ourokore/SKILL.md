...
1. **控制區塊與 Handle 代數系統 (ControlBlock & Handle System)**：
   ...
   - `WeakHandle<T>`：非擁有型引用，專為動態模組/DLL 插件熱卸載防釘死、生命週期解耦與旁路觀察設計，支援安全鎖定 (`LockAndAcquire()`)，具備惰性修剪（Lazy Pruning）機制，徹底杜絕懸掛野指標與模組生命週期被釘死之缺陷。
...
public:
    ...
    ork::WeakHandle<ork::OuroObject>    m_plugin_module; // 弱引用，防止釘死動態 DLL
