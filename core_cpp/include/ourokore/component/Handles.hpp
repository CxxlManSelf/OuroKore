...
/**
 * @brief UnboundHandle（旁觀者句柄）：純觀察、不干涉對方生死的弱引用句柄。
 *
 * 【白話心智模型】
 * * OwningHandle 像是「主管或父母」：你是我的組員/小孩，我為你的生死負責。只要我還需要你，系統就不能把你刪掉。
 * * UnboundHandle 像是「通訊錄」：我只把你的電話號碼（ID）記在小本本上。你隨時想離職或被公司開除，我都絕不霸佔你讓你走不掉。
 *
 * 【什麼時候該用？最常見的兩大情境】
 * 1. 外掛隨時卸載防卡死：
 *    例如主角身上掛了一個「第三方外掛插件提供的 AI 腳本」。如果用強引用抓著它，當玩家要關閉外掛或更新 DLL 時，
 *    就會因為被主角「釘住不放」而卡住卸不掉甚至當機。用 UnboundHandle，外掛隨時可以自己退出下班，主角絕不阻止。
 * 2. UI 視窗或快取瞄一眼（只看不管養）：
 *    UI 介面顯示當前選取怪物的血條。怪物死掉就該正常進回收流程，UI 不該強行把怪物屍體扣在記憶體裡。
 *    UI 只要用 UnboundHandle 瞄一眼，怪物沒了 UI 就關閉視窗。
 *
 * 【使用方式：打電話確認（LockAndAcquire）】
 * * 因為對方隨時可能離開，不能直接拿來呼叫方法。
 * * 每次要用的時候呼叫 `.LockAndAcquire()`：
 *   - 對方還在：暫時拿到一把安全的短期保護鎖（OuroPtr<T>），期間對方絕不會突然消失。
 *   - 對方已走：傳回 null，並自動把通訊錄擦乾淨（下次就不用白問了）。
 *
 * ⚠️【避坑鐵律：切勿拿它來破環！】
 * 在傳統 C++ (std::shared_ptr) 中大家習慣用 weak_ptr 打破雙向循環死結；
 * 但在 OuroKore 中完全不需要！物件互相強引用（OwningHandle）由底層背景清潔工（CycleCollector）自動回收。
 * 只有在「完全不想為對方的生命週期負責」時才使用 UnboundHandle。
 */
template <typename T>
class UnboundHandle
{
