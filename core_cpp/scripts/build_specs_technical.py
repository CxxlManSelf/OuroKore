...
## 🛡️ 3. OOM 被動自救與 Passkey 權杖防禦 (Passkey & Context Guard)

* **透明換頁自救**：當第三方插件透過 `CreateObject<T>()` 配置記憶體遭遇 `std::bad_alloc` 時，核心具備被動換頁釋放冷物件之能力。
* **雙重權限防護**：
  1. **編譯期 Passkey 防禦**：底層救援函式強制要求私有權杖 `OuroCreationToken`，私有建構子僅對合法建構樣板授權，任何外部程式碼無法直接實例化，徹底杜絕外掛主動越權調用。
  2. **執行期情境驗證**：核心在觸發脫水前，驗證 HandleID 是否正處於兩階段預留中或脫水重建狀態（`IsValidRescueContext`），非建構情境之非法呼叫一律安全拒絕。
  3. **私有封裝復水器 (Private Rehydrator)**：底層復水函式與回呼簽章為 `void (*)(HandleID)`，由 `detail::Rehydrator` 進行 private static 封裝，僅對公開合法 API 開放友元授權，徹底杜絕第三方插件獲取物件內部裸指標或干擾復水流程。
''', encoding="utf-8")
